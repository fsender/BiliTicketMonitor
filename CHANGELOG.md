...
## Release 3.1.0 - 2026/4/29

1. 修复监控选定票档时的钩子函数调用问题.

## Release 3.1.0 - 2026/4/28

1. **极致性能优化**: curl_multi 事件循环零 CPU 阻塞等待 (`wait(50)`), 匹配 Python 原生性能
2. **单请求微秒级延迟显示**: 每轮显示本轮最慢请求耗时 (μs), 即时掌握网络状态
3. **格式化状态栏**: 请求数/周期数 9 位对齐, 延迟 4 位对齐
4. 移除冗余的事件循环 busy-wait, 大幅降低 CPU 占用
5. HttpMulti 句柄跨周期复用, TCP 连接存活率提升
